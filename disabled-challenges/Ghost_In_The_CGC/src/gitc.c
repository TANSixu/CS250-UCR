...
void cgc_heapProtect()
{
  ...
  while (1)
  {
    ret = cgc_allocate(0x1000*PAGE_SIZE, 0, &temp);
    if (ret != 0)
    {
      break;
    }
  ...
  while (1)
  {
    ret = cgc_allocate(0x100*PAGE_SIZE, 0, &temp);
    if (ret != 0)
    {
      break;
    }
  ...
  while (1)
  {
    ret = cgc_allocate(0x10*PAGE_SIZE, 0, &temp);
    if (ret != 0)
    {
      break;
    }
  ...
  while (1)
  {
    ret = cgc_allocate(0x1*PAGE_SIZE, 0, &temp);
    if (ret != 0)
    {
      break;
    }
...
int cgc_curErrorLevel = 0;

unsigned int cgc_curVal = 0;

int cgc_gSeed = 0;
//MAX_OPTIONS is 10, so this will work out fine. 
int cgc_rand()
{
  cgc_gSeed = (cgc_gSeed + 7) % 13;
  return (cgc_gSeed);
}
...
void cgc_printLights(void* addr)
{
  uint8_t contents = 0;
  char temp[10] = "BBBBBBBB\n";
  int i = 0;

  if (cgc_isValidAddr(addr))
  {
...
int main(int cgc_argc, char *cgc_argv[])
{
  ...
  while (curNode != NULL)
  {
    if (bValidResponse)
    {
      i = cgc_rand() % curNode->numOps;
      cgc_fprintStr(STDOUT, curNode->m[i]);
      cgc_fprintStr(STDOUT, "\n");
    }
...
#ifdef PATCHED_1
    numRead = cgc_receive_until(STDIN, buf, sizeof(buf), '\n');
#else
    numRead = cgc_receive_until(STDIN, buf, sizeof(buf) * 2, '\n');
#endif 
    if (numRead == 0)
    {
      break;
    ...
    {
      addr = cgc_hexStrToUint32(buf);
      cgc_printLights((void*)addr); 
    ...
    {
      if (cgc_strcmp(curNode->responses[j], buf) == 0)
      {
    ...
    if (!bValidResponse)
    {
      cgc_fprintStr(STDOUT, errorMsgs[cgc_curErrorLevel]);
      cgc_fprintStr(STDOUT, "\n");
      cgc_curErrorLevel++;
      if (cgc_curErrorLevel > 2)
      { 
        return (0);  
      }
    }
