...
def getDumps(inFile_path, outFile_path, dumpCmd_path, dumpCmd_options, bForce = False) :
  if bForce or not os.path.exists(outFile_path) :
    ...
    ret = 0
    try :
      ret = subprocess.call([dumpCmd_path] + dumpCmd_options + [inFile_path], cgc_stdout=outFile)
    except OSError :
      ret = -1
...
def getDiffFile(bForce = False) :
  ...
  ret = 0
  try :
    ret = subprocess.call(["/usr/bin/diff", cb_dump_path, patched_dump_path], cgc_stdout=outFile)
  except OSError :
    ret = -1 
...
def getHDDiffFile(bForce = False) :
  ...
  ret = 0
  try :
    ret = subprocess.call(["/usr/bin/diff", cb_hd_path, patched_hd_path], cgc_stdout=outFile)
  except OSError :
    ret = -1 
